...
- `rawDescription` (Text): Raw text/voice transcript entered by user
...
All entities reside within a single DynamoDB table named `ServiceForge_CoreTable`:

| Entity | Partition Key (PK) | Sort Key (SK) | GSI1-PK | GSI1-SK |
| :--- | :--- | :--- | :--- | :--- |
| **Customer** | `CUST#<CustomerId>` | `METADATA` | `ORG#<OrgId>` | `CUST#<CustomerId>` |
| **Asset** | `CUST#<CustomerId>` | `ASSET#<AssetId>` | `ASSET#<AssetId>` | `METADATA` |
| **ServiceRequest** | `REQ#<RequestId>` | `METADATA` | `CUST#<CustomerId>` | `CREATED#<Timestamp>` |
| **AIAnalysis** | `REQ#<RequestId>` | `AI_ANALYSIS` | `AI#<AnalysisId>` | `METADATA` |
| **ServiceJob** | `JOB#<JobId>` | `METADATA` | `STATUS#<Status>` | `SLA#<Deadline>` |
| **JobAssignment** | `JOB#<JobId>` | `ASSIGN#<AssignId>` | `TECH#<TechnicianId>` | `SCHEDULED#<Time>` |
| **JobUpdate** | `JOB#<JobId>` | `UPDATE#<Timestamp>` | `TECH#<TechnicianId>` | `UPDATE#<Timestamp>` |
| **ServiceReport** | `JOB#<JobId>` | `REPORT` | `CUST#<CustomerId>` | `REPORT#<ReportId>` |
